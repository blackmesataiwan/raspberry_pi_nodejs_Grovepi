...
var coapConnection = {
    host: '172.17.28.20',
    port:"5683", //COAP UDP port
    pathname: 'r/qiot/things/admin/test/test', //topicname
    method: 'PUT',
    observe: false,
    confirmable: true,
    query: 'r=6df6a142-5421-4d50-8321-17f035a5a4ab&t=r:fc943307c59818e63b77aec90ca56a8e' //r stands for requestorid and t stands for access-token.
  }
...
function getresourceinfo(fileName) {
        ...
        coapConnection.query = "r="+USER_NAME+"&t="+USER_PASS;
    ...
    id:     function (id, value, Qsend){
                for (var sensoridx in sensors) {
                    if (id == sensors[sensoridx].id) {
                        ...
                        Qsend.pathname = 'r/' + sensors[sensoridx].topic;
                        Qsend.method = 'PUT';
                        var qiot_value = sensors[sensoridx].value;

                        var req = coap.request(Qsend).end('{"value":'+qiot_value+'}'); // message Payload with random string
                        ...
                        console.log(" send message to [coap://" + HOST + "/r/" + topic_Pub + ", value = " + JSON.stringify({value: qiot_value}) + sensors[sensoridx].name);
                    }
                    else{
                ...
                for (var sensoridx in sensors) {
                    if (restype_name == sensors[sensoridx].resourcetype) {
                        ...
                        req.on('response', function(res) {
                            res.pipe(process.stdout);
                            if(res.code=="2.04") //Success Code Handler
                              console.log(" send message to [coap://" + HOST +  Qsend.pathname + ", value = " + JSON.stringify({value: qiot_value}) + sensors[sensoridx].name);
                            else //Error Code Handler
                              console.log("Error Code:  "+res.code);
                        })                        
                    }
    ...
    subscribeofid: function(id, Qreceive){
                    for (var sensoridx in sensors) {
                        if (id == sensors[sensoridx].id) {
                            ...
                            Qreceive.pathname = 'r/' + sensors[sensoridx].topic;
                            Qreceive.method = 'GET';
